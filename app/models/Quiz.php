...
class Quiz {
  ...
  public function upsertBySection(
      ...
      $stmt = $this->db->prepare(
        'UPDATE quizzes
         SET title=?, instructions=?, time_limit_minutes=?, available_from=?, due_at=?,
             passing_score=?, show_results=?, is_exam=?
         WHERE id=?'
      );
      $stmt->bind_param(
        'ssissisii',
        $title, $instructions, $time_limit_minutes, $available_from, $due_at,
        $passing_score, $show_results, $is_exam, $id
      );
    ...
    $stmt = $this->db->prepare(
      'INSERT INTO quizzes (section_id, title, instructions, time_limit_minutes, available_from, due_at, passing_score, show_results, is_exam)
       VALUES (?,?,?,?,?,?,?,?,?)'
    );
    $stmt->bind_param(
      'ississisi',
      $section_id, $title, $instructions, $time_limit_minutes, $available_from, $due_at,
      $passing_score, $show_results, $is_exam
    );
    if (!$stmt->execute()) return false;
