...
CREATE TABLE IF NOT EXISTS messages (
  ...
  to_role ENUM('ADMIN','DOCENTE','ESTUDIANTE') DEFAULT NULL,
  asunto VARCHAR(200) NOT NULL,
  cuerpo TEXT NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  read_at TIMESTAMP NULL DEFAULT NULL,
  PRIMARY KEY (id),
  KEY fk_msg_from (from_user_id),
...
CREATE TABLE IF NOT EXISTS reports (
  ...
  report_type ENUM('ACADEMIC_NOTES','ACADEMIC_ATTENDANCE','ADMIN_PAYMENTS','ADMIN_ENROLLMENTS_LEVEL','ADMIN_ENROLLMENTS_SECTION') NOT NULL,
  scope_role ENUM('ADMIN','DOCENTE') NOT NULL,
  filters_json JSON NOT NULL,
  created_by_user_id INT NOT NULL,
  updated_by_user_id INT NOT NULL,
  created_at TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT fk_reports_created_by FOREIGN KEY (created_by_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT fk_reports_updated_by FOREIGN KEY (updated_by_user_id) REFERENCES users(id) ON UPDATE CASCADE ON DELETE CASCADE
) ENGINE=InnoDB;
